<?php

// error_reporting(E_ALL);
// ini_set('display_errors', 1);
// ini_set('error_reporting', E_ALL); 

$base = dirname(__FILE__).'/';

include($base.'../inc/db.php');
include($base.'../inc/functions.php');
include($base.'../inc/class.phpclicolors.php');

$colors = new Colors();

$task = $argv[1];

if($task == 'import')
{
	$query 		= "SELECT * FROM `email_lists` WHERE `status` = 'pending_import' ORDER BY `id` LIMIT 1";
	$result 	= mysql_query($query) or die(mysql_error());
	$records	= mysql_num_rows($result);
	while($row = mysql_fetch_array($result)){
		$data['id'] 					= $row['id'];
		$data['user_id']				= $row['user_id'];
		$data['name']					= stripslashes($row['name']);
		$data['filename']				= $row['filename'];
		
		console_output('List ID: '.$data['id'].' | List Name:'.$data['name']);
		
		if(file_exists('/home/inboxmissile/public_html/app/list_uploads/'.$data['user_id'].'/'.$data['filename'])){
			console_output('File exists, proceeding');
			$sql="
			LOAD DATA LOCAL INFILE '/home/inboxmissile/public_html/app/list_uploads/".$data['user_id']."/".$data['filename']."' INTO TABLE `emails` 
			FIELDS 
				TERMINATED BY ',' 
				ENCLOSED BY '\"' 
			LINES 
				TERMINATED BY '\n' 
			(@col1) 
			SET `email` = @col1, `email_md5` = MD5(@col1), `user_id` = '".$data['user_id']."', `list_id` = '".$data['id']."', `status` = 'pending' ";
			$result = mysql_query($sql) or die(mysql_error());
			
			mysql_query("UPDATE `email_lists` SET `status` = 'pending_clean' WHERE `id` = '".$data['id']."' ") or die(mysql_error());
			
			$query 		= "SELECT `id` FROM `emails` WHERE `list_id` = '".$data['id']."' ";
			$result 	= mysql_query($query) or die(mysql_error());
			$data['emails_total']	= mysql_num_rows($result);
			mysql_query("UPDATE `email_lists` SET `emails_total` = '".$data['emails_total']."' WHERE `id` = '".$data['id']."' ") or die(mysql_error());
			
			mysql_query("UPDATE `emails` SET `email`=replace(convert(`email` using utf8) ,'\r','') WHERE `list_id` = '".$data['id']."' ") or die(mysql_error());
			
			mysql_query("DELETE FROM `emails` WHERE `email` = 'email' OR `email` = 'emailaddress' OR `email` = 'email address' OR `email` = 'EMAIL' OR `email` = 'EMAILADDRESS' OR `email` = 'EMAIL ADDRESS' OR `email` = 'address' OR `email` = 'ADDRESS'") or die(mysql_error());

			// shell_exec("rm -rf /home/inboxmissile/public_html/app/list_uploads/".$data['user_id']."/".$data['filename']);
		}else{
			console_output('File DOES NOT exist, terminating');
		}
		
	}
	
}

if($task == 'validate_multi')
{
	$records 				= 10;
	$search_records 		= $records;
	
	// get the latest job
	$query 		= "SELECT * FROM `jobs` WHERE `status` = 'pending' ORDER BY `id` LIMIT 1";
	$result 	= mysql_query($query) or die(mysql_error());
	$number_of_jobs	= mysql_num_rows($result);
	if($number_of_jobs == 0){
		console_output('No pending jobs.');
	}else{
		// include_once '../inc/class.verifyEmail.php';
		
		while($row = mysql_fetch_array($result)){
			$data['id'] 					= $row['id'];
			$data['user_id']				= $row['user_id'];
			$data['list_id']				= $row['list_id'];
		}
		
		$query_1 		= "SELECT `id`,`email` FROM `emails` WHERE `list_id` = '".$data['list_id']."' AND `status` = 'pending' ORDER BY `id` LIMIT ".$records;
		$result_1 		= mysql_query($query_1) or die(mysql_error());
		$count 			= mysql_num_rows($result_1);
		
		if ( $count == 0 ) {
			mysql_query("UPDATE `jobs` SET `status` = 'complete' WHERE `id` = '".$data['id']."' ") or die(mysql_error());
			mysql_query("UPDATE `email_lists` SET `status` = 'cleaned' WHERE `id` = '".$data['list_id']."' ") or die(mysql_error());
		
			console_output("Job ID: ".$data['id']." finished.");
			die();
		}
		
		console_output('User ID: '.$data['user_id']);
		console_output('Job ID: '.$data['id']);
		console_output('List ID: '.$data['list_id']);
		
		console_output("Starting Job Run");
		
		console_output(" ");
		
		$pids = array();
				
		for($i = 0; $i < $records; $i++) {
			
			$pids[$i] = pcntl_fork();

			if(!$pids[$i]) {
				include($base.'../inc/db.php');
				$query_2 		= "SELECT * FROM `nodes` WHERE `status` = 'active' ORDER BY RAND() LIMIT 1";
				$result_2 		= mysql_query($query_2) or die(mysql_error());
				while($row_2 = mysql_fetch_array($result_2)){
					$node['id']					= $row_2['id'];
					$node['name']				= $row_2['name'];
					$node['url']				= $row_2['url'];
				}
				
				console_output();
				
				$query_1 		= "SELECT `id`,`email` FROM `emails` WHERE `list_id` = '".$data['list_id']."' AND `status` = 'pending' ORDER BY RAND() LIMIT 1";
				$result_1 		= mysql_query($query_1) or die(mysql_error());
				$count 			= mysql_num_rows($result_1);

				$count = 0;

				while($row_1 = mysql_fetch_array($result_1)){
					$email['id'] 		= $row_1['id'];
					$email['email'] 	= $row_1['email'];

					$bits = explode("@", $row_1['email']);
					$email['domain'] = $bits[1];
				}
				
				// check role account
				if ( check_role_account ( $email['email'] == 'no_match' ) ) {

				} else {
					console_output(
						$colors->getColoredString(
							$node['name'] . ' > ' . $email['email']." failed ROLE ACCOUNT check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'roleaccount_fail' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// validate email syntax
				if ( valid_email ( $email['email'] ) ) {

				} else {
					console_output(
						$colors->getColoredString(
							$node['name'] . ' > ' . $email['email']." failed SYNTAX check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'syntax_fail' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// check internal blacklist
				if(check_int_blacklist($email['email']) == 'clean'){

				}else{
					console_output(
						$colors->getColoredString(
							$node['name'] . ' > ' . $email['email']." failed BLACKLIST check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'blacklist_fail' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// validate domain
				if(checkdnsrr($email['domain'], 'ANY')){

				}else{
					console_output(
						$colors->getColoredString(
							$node['name'] . ' > ' . $email['email']." failed DOMAIN check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'domain_fail' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// validate mx
				if(checkdnsrr($email['domain'], 'MX')){

				}else{
					console_output(
						$colors->getColoredString(
							$node['name'] . ' > ' . $email['email']." failed MX check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'mx_fail' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// smtp check

				$node_data = file_get_contents($node['url'].$email['email']);
				$node_data = json_decode($node_data, true);

				if($node_data['result'] == 'exists'){
					console_output(
						$colors->getColoredString(
							$node['name'] . ' > ' . $email['email']." PASSED ALL CHECKS.", 
						"green", "black"));

					mysql_query("UPDATE `emails` SET `status` = 'pass' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
				}else{
					console_output(
						$colors->getColoredString(
							$node['name'] . ' > ' . $email['email']." failed SMTP check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'smtp_fail' WHERE `id` = '".$email['id']."' ") or die(mysql_error());

					$input = mysql_query("INSERT IGNORE INTO `global_blacklist` 
					(`email`, `reason`)
					VALUE
					('".$email['email']."', 'failed inboxmissile.com smtp check')") or die(mysql_error());

				}
				
				mysql_disconnect();
				
				exit();
			}
		}
		
		for($i = 0; $i < $records; $i++) {
			pcntl_waitpid($pids[$i], $status, WUNTRACED);
		}
	}
}

if($task == 'validate_new') 
{
	// load cron lock helper
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) {
		$records                = $argv[2];
		$search_records         = $records;

		// get the latest job
		$query      = "SELECT * FROM `jobs` WHERE `status` = 'pending' ORDER BY `id` LIMIT 1";
		$result     = mysql_query($query) or die(mysql_error());
		$number_of_jobs = mysql_num_rows($result);
		if($number_of_jobs == 0){
			console_output('No pending jobs.');
		}else{

			while($row = mysql_fetch_array($result)){
				$data['id']                     = $row['id'];
				$data['user_id']                = $row['user_id'];
				$data['list_id']                = $row['list_id'];
			}

			$query_1        = "SELECT `id`,`email` FROM `emails` WHERE `list_id` = '".$data['list_id']."' AND `status` = 'pending' ORDER BY `id` LIMIT ".$records;
			$result_1       = mysql_query($query_1) or die(mysql_error());
			$count          = mysql_num_rows($result_1);
			if ( $count == 0 ){
				mysql_query("UPDATE `email_lists` SET `job_starttime` = '".time()."' WHERE `id` = '".$data['list_id']."' ") or die(mysql_error());

				mysql_query("UPDATE `jobs` SET `status` = 'complete' WHERE `id` = '".$data['id']."' ") or die(mysql_error());

				console_output('Job has finished.');
			}
			while($row_1 = mysql_fetch_array($result_1)){
				$data['emails'][$count]['id']       = $row_1['id'];
				$data['emails'][$count]['email']    = $row_1['email'];

				$bits = explode("@", $row_1['email']);
				$data['emails'][$count]['domain'] = $bits[1];

				$count++;
			}

			console_output('User ID: '.$data['user_id']);
			console_output('Job ID: '.$data['id']);
			console_output('List ID: '.$data['list_id']);
			console_output("Segment Size: ".number_format(count($data['emails'])));

			$records = count($data['emails']);

			console_output("Starting Job Run");

			console_output(" ");
			
			$count = count ( $data['emails'] );

			foreach($data['emails'] as $email){
				// run internal tests first
				// check role account
				if ( check_role_account ( $email['email'] == 'no_match' ) ) {

				} else {
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed ROLE ACCOUNT check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'roleaccount' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// validate email syntax
				if ( valid_email ( $email['email'] ) ) {

				} else {
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed SYNTAX check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'invalid_syntax' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// check internal blacklist
				if(check_int_blacklist($email['email']) == 'clean'){

				}else{
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed BLACKLIST check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'blacklisted_internal' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// validate domain
				if(checkdnsrr($email['domain'], 'ANY')){

				}else{
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed DOMAIN check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'domain_does_not_exist' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// validate mx
				if(checkdnsrr($email['domain'], 'MX')){

				}else{
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed MX check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'mxserver_does_not_exist' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}


				// run external checks
				// bulkemailchecker.com API call
				$key = 'mbJ56dBNykfGQKjLhvoESstHcDV0eIuF';

				// use curl to make the request
				$url = 'http://bulk-api.bulkemailchecker2.com/?key='.$key.'&email='.urlencode($email['email']);
				$ch = curl_init($url);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 15); 
				curl_setopt($ch, CURLOPT_TIMEOUT, 15); 
				$response = curl_exec($ch);
				curl_close($ch);

				// decode the json response
				$json = json_decode($response, true);

				// if address is failed, alert the user they entered an invalid email
				if($json['status'] == 'failed'){
					// email failed				
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." marked as ".$json['event'], 
						"red", "black"));

					mysql_query("UPDATE `emails` SET `status` = '".$json['event']."' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					
					$input = mysql_query("INSERT IGNORE INTO `global_blacklist` 
					(`email`, `reason`)
					VALUE
					('".$email['email']."', 'bulkemailchecker.com ".$json['event']."')") or die(mysql_error());
				}elseif($json['status'] == 'passed'){
					// email passed
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." PASSED ALL CHECKS.", 
						"green", "black"));

					mysql_query("UPDATE `emails` SET `status` = '".$json['event']."' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
				}else{
					// email status unknown
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." status in unknown - ".$json['event'], 
						"blue", "black"));

					mysql_query("UPDATE `emails` SET `status` = 'unknown' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
				}	
				
				$count = $count - 1;
			}
		}
	}else{
		console_output(
			$colors->getColoredString(
				"Script already running", 
			"pink", "black"));
	}
}

if($task == 'clean')
{
	// load cron lock helper
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) {
		$records                = $argv[2];
		$search_records         = $records;

		// get the latest job
		$query      = "SELECT * FROM `jobs` WHERE `status` = 'pending_clean' ORDER BY `id` LIMIT 1";
		$result     = mysql_query($query) or die(mysql_error());
		$number_of_jobs = mysql_num_rows($result);
		if($number_of_jobs == 0){
			console_output('No pending jobs.');
		}else{

			while($row = mysql_fetch_array($result)){
				$data['id']                     = $row['id'];
				$data['user_id']                = $row['user_id'];
				$data['list_id']                = $row['list_id'];
			}

			$query_1        = "SELECT `id`,`email` FROM `emails` WHERE `list_id` = '".$data['list_id']."' AND `status` = 'pending' ORDER BY `id` LIMIT ".$records;
			$result_1       = mysql_query($query_1) or die(mysql_error());
			$count          = mysql_num_rows($result_1);
			if ( $count == 0 ){
				mysql_query("UPDATE `email_lists` SET `job_starttime` = '".time()."' WHERE `id` = '".$data['list_id']."' ") or die(mysql_error());

				mysql_query("UPDATE `jobs` SET `status` = 'complete' WHERE `id` = '".$data['id']."' ") or die(mysql_error());

				console_output('Job has finished.');
			}
			while($row_1 = mysql_fetch_array($result_1)){
				$data['emails'][$count]['id']       = $row_1['id'];
				$data['emails'][$count]['email']    = $row_1['email'];

				$bits = explode("@", $row_1['email']);
				$data['emails'][$count]['domain'] = $bits[1];

				$count++;
			}

			console_output('User ID: '.$data['user_id']);
			console_output('Job ID: '.$data['id']);
			console_output('List ID: '.$data['list_id']);
			console_output("Segment Size: ".number_format(count($data['emails'])));

			$records = count($data['emails']);

			console_output("Starting Job Run");

			console_output(" ");
			
			$count = count ( $data['emails'] );

			foreach($data['emails'] as $email){
				// run internal tests first
				// check role account
				if ( check_role_account ( $email['email'] == 'no_match' ) ) {

				} else {
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed ROLE ACCOUNT check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'roleaccount' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// validate email syntax
				if ( valid_email ( $email['email'] ) ) {

				} else {
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed SYNTAX check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'invalid_syntax' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// check internal blacklist
				if(check_int_blacklist($email['email']) == 'clean'){

				}else{
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed BLACKLIST check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'blacklisted_internal' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				// domain check
				$domain_check = check_domain ( $email['email'] );
				if ( $domain_check['status'] == 'mxserver_does_not_exist' ) {
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed MX check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'mxserver_does_not_exist' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}
				if ( $domain_check['status'] == 'domain_does_not_exist' ) {
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $email['email']." failed DOMAIN check.", 
						"red", "black"));
					mysql_query("UPDATE `emails` SET `status` = 'domain_does_not_exist' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
					continue;
				}

				console_output(
					$colors->getColoredString(
						number_format($count) . ') ' . $email['email']." PASSED ALL CHECKS.", 
					"green", "black"));

				mysql_query("UPDATE `emails` SET `status` = 'clean' WHERE `id` = '".$email['id']."' ") or die(mysql_error());	
				
				$count = $count - 1;
			}
		}
	}else{
		console_output(
			$colors->getColoredString(
				"Script already running", 
			"pink", "black"));
	}
}

if($task == 'clean_multi')
{

	$records                = $argv[2];
	$threads 				= $argv[3];
	
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) 
	{
		
		console_output("Spawning ".$threads." children.");
		
		$pids = array();
				
		for ( $i = 0; $i < $threads; $i++ ) 
		{
			
			$pids[$i] = pcntl_fork();

			if ( !$pids[$i] ) 
			{
				
				include($base.'../inc/db.php');
				
				// get the latest job
				$query      = "SELECT * FROM `jobs` WHERE `status` = 'pending_clean' ORDER BY `id` LIMIT 1";
				$result     = mysql_query($query) or die(mysql_error());
				$number_of_jobs = mysql_num_rows($result);
				if($number_of_jobs == 0){
					console_output('No pending jobs.');
				}else{

					while($row = mysql_fetch_array($result)){
						$data['id']                     = $row['id'];
						$data['user_id']                = $row['user_id'];
						$data['list_id']                = $row['list_id'];
					}

					$query_1        = "SELECT `id`,`email` FROM `emails` WHERE `list_id` = '".$data['list_id']."' AND `status` = 'pending' ORDER BY RAND() LIMIT ".$records;
					$result_1       = mysql_query($query_1) or die(mysql_error());
					$count          = mysql_num_rows($result_1);
					if ( $count == 0 ){
						mysql_query("UPDATE `email_lists` SET `job_endtime` = '".time()."' WHERE `id` = '".$data['list_id']."' ") or die(mysql_error());
						mysql_query("UPDATE `email_lists` SET `status` = 'complete' WHERE `id` = '".$data['list_id']."' ") or die(mysql_error());
						mysql_query("UPDATE `email_lists` SET `cleaned` = 'yes' WHERE `id` = '".$data['list_id']."' ") or die(mysql_error());

						mysql_query("UPDATE `jobs` SET `status` = 'complete' WHERE `id` = '".$data['id']."' ") or die(mysql_error());

						console_output('Job has finished.');
					}
					while($row_1 = mysql_fetch_array($result_1)){
						$data['emails'][$count]['id']       = $row_1['id'];
						$data['emails'][$count]['email']    = $row_1['email'];

						$bits = explode("@", $row_1['email']);
						$data['emails'][$count]['domain'] = $bits[1];

						$count++;
					}

					console_output('User ID: '.$data['user_id']);
					console_output('Job ID: '.$data['id']);
					console_output('List ID: '.$data['list_id']);
					console_output("Segment Size: ".number_format(count($data['emails'])));

					$records = count($data['emails']);

					console_output("Starting Job Run");

					console_output(" ");

					$count = count ( $data['emails'] );

					foreach($data['emails'] as $email){
						// run internal tests first
						// check role account
						if ( check_role_account ( $email['email'] == 'no_match' ) ) {

						} else {
							console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $email['email']." failed ROLE ACCOUNT check.", 
								"red", "black"));
							mysql_query("UPDATE `emails` SET `status` = 'roleaccount' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
							continue;
						}

						// validate email syntax
						if ( valid_email ( $email['email'] ) ) {

						} else {
							console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $email['email']." failed SYNTAX check.", 
								"red", "black"));
							mysql_query("UPDATE `emails` SET `status` = 'invalid_syntax' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
							continue;
						}

						// check internal blacklist
						if(check_int_blacklist($email['email']) == 'clean'){

						}else{
							console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $email['email']." failed BLACKLIST check.", 
								"red", "black"));
							mysql_query("UPDATE `emails` SET `status` = 'blacklisted_internal' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
							continue;
						}

						// domain check
						$domain_check = check_domain ( $email['email'] );
						if ( $domain_check['status'] == 'mxserver_does_not_exist' ) {
							console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $email['email']." failed MX check.", 
								"red", "black"));
							mysql_query("UPDATE `emails` SET `status` = 'mxserver_does_not_exist' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
							continue;
						}
						if ( $domain_check['status'] == 'domain_does_not_exist' ) {
							console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $email['email']." failed DOMAIN check.", 
								"red", "black"));
							mysql_query("UPDATE `emails` SET `status` = 'domain_does_not_exist' WHERE `id` = '".$email['id']."' ") or die(mysql_error());
							continue;
						}

						console_output(
							$colors->getColoredString(
								number_format($count) . ') ' . $email['email']." PASSED ALL CHECKS.", 
							"green", "black"));

						mysql_query("UPDATE `emails` SET `status` = 'clean' WHERE `id` = '".$email['id']."' ") or die(mysql_error());	

						$count = $count - 1;
					}
				}
				
				mysql_disconnect();
				
				exit();
			}
		}
		
		for ( $i = 0; $i < $threads; $i++ ) 
		{
			pcntl_waitpid($pids[$i], $status, WUNTRACED);
		}
	}else{
		console_output(
			$colors->getColoredString(
				"Script already running", 
			"pink", "black"));
	}
}

if($task == 'domain_checker')
{
	// load cron lock helper
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) {
		$records                = $argv[2];
		$search_records         = $records;

		$query      = "SELECT * FROM `email_domains` WHERE 
				`last_checked` IS NULL OR 
				`last_checked` = '0'
				LIMIT 10
				";
		$result     = mysql_query($query) or die(mysql_error());
		$count 		= mysql_num_rows($result);
		
		while($row = mysql_fetch_array($result)){
			$data[$count]['id']                    = $row['id'];
			$data[$count]['domain']                = $row['domain'];
			$data[$count]['status']                = $row['status'];
			
			if(checkdnsrr($data[$count]['domain'], 'ANY')){
				// domain exists, lets set to active then check MX records
				$update = $db->exec("UPDATE `email_domains` SET `status` = 'active' WHERE `id` = '".$data[$count]['id']."' ");

				// validate mx
				if(checkdnsrr($data[$count]['domain'], 'MX')){
					// do nothing, its active and has a valid MX record
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $data[$count]['domain'] . " is active and has valid MX record.", 
						"green", "black"));
				}else{
					console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $data[$count]['domain'] . " is active but has no MX record.", 
						"blue", "black"));
					$update = $db->exec("UPDATE `email_domains` SET `status` = 'mxserver_does_not_exist' WHERE `id` = '".$data[$count]['id']."' ");

				}
			} else {
				// domain does not exist
				console_output(
						$colors->getColoredString(
							number_format($count) . ') ' . $data[$count]['domain'] . " is not active.", 
						"red", "black"));
				$update = $db->exec("UPDATE `email_domains` SET `status` = 'domain_does_not_exist' WHERE `id` = '".$data[$count]['id']."' ") or die(mysql_error());
			}

			$update = $db->exec("UPDATE `email_domains` SET `last_checked` = '".time()."' WHERE `id` = '".$data[$count]['id']."' ") or die(mysql_error());
			
			$count = $count - 1;
		}
	}
}

if($task == 'domain_checker_multi')
{
	$records                = $argv[2];
	$threads 				= $argv[3];
	
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) 
	{
		
		console_output("Spawning ".$threads." children and pulling " . number_format ( $records ) . " records." );
		
		$pids = array();
				
		for ( $i = 0; $i < $threads; $i++ ) 
		{
			
			$pids[$i] = pcntl_fork();

			if ( !$pids[$i] ) 
			{
				include($base.'../inc/db.php');
				
				$query      = "SELECT * FROM `domains` WHERE 
				`last_checked` IS NULL OR 
				`last_checked` = '1' 
				ORDER BY RAND() LIMIT ".$records;
				$result     = mysql_query($query) or die(mysql_error());
				$count 		= mysql_num_rows($result);

				while($row = mysql_fetch_array($result)){
					$data[$count]['id']                    = $row['id'];
					$data[$count]['domain']                = $row['domain'];
					$data[$count]['status']                = $row['status'];

					if(checkdnsrr($data[$count]['domain'], 'ANY')){
						// domain exists, lets set to active then check MX records
						mysql_query("UPDATE `domains` SET `status` = 'active' WHERE `id` = '".$data[$count]['id']."' ") or die(mysql_error());

						// validate mx
						if(checkdnsrr($data[$count]['domain'], 'MX')){
							// do nothing, its active and has a valid MX record
							console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $data[$count]['domain'] . " is active and has valid MX record.", 
								"green", "black"));
						}else{
							console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $data[$count]['domain'] . " is active but has no MX record.", 
								"blue", "black"));
							mysql_query("UPDATE `domains` SET `status` = 'mxserver_does_not_exist' WHERE `id` = '".$data[$count]['id']."' ") or die(mysql_error());

						}
					} else {
						// domain does not exist
						console_output(
								$colors->getColoredString(
									number_format($count) . ') ' . $data[$count]['domain'] . " is not active.", 
								"red", "black"));
						mysql_query("UPDATE `domains` SET `status` = 'domain_does_not_exist' WHERE `id` = '".$data[$count]['id']."' ") or die(mysql_error());
					}
					
					mysql_query("UPDATE `domains` SET `last_checked` = '".time()."' WHERE `id` = '".$data[$count]['id']."' ") or die(mysql_error());
					
					$count = $count - 1;
				}
				
				mysql_disconnect();
				
				exit();
			}
		}
		
		for ( $i = 0; $i < $threads; $i++ ) 
		{
			pcntl_waitpid($pids[$i], $status, WUNTRACED);
		}
	}
}

if($task == 'bulk_import_bounces')
{
	$sql="
	LOAD DATA LOCAL INFILE 'file.csv' IGNORE INTO TABLE `global_blacklist` 
	FIELDS 
		TERMINATED BY ',' 
		ENCLOSED BY '\"' 
	LINES 
		TERMINATED BY '\n' 
	(@col1, @cal2) 
	SET `email` = @col1, `reason` = @col2
	";

	$result = mysql_query($sql) or die(mysql_error());
}

if($task == 'update_totals')
{
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) {
		$query 		= "SELECT `status` AS 'email_status' FROM `emails` GROUP BY `status` ";
		$result		= mysql_query($query) or die(mysql_error());
		while($row = mysql_fetch_array($result)){
			$status_values[]			= $row['email_status'];
		}

		$query_1 		= "SELECT * FROM `email_lists` ORDER BY `id` ";
		$result_1 		= mysql_query($query_1) or die(mysql_error());
		while($row_1 = mysql_fetch_array($result_1)){
			$data['id'] 				= $row_1['id'];
			$data['name'] 				= stripslashes($row_1['name']);

			console_output('List ID: '.$data['id']);
			console_output('List Name: '.$data['name']);

			foreach ( $status_values as $status_value ) {


				$query_2 					= "SELECT `id` FROM `emails` WHERE `list_id` = '".$data['id']."' AND `status` = '".$status_value."' ";
				$result_2 					= mysql_query($query_2) or die(mysql_error());
				$total						= mysql_num_rows($result_2);

				$query_3					= "UPDATE `email_lists` SET `".$status_value."` = '".$total."' WHERE `id` = '".$data['id']."' ";
				mysql_query($query_3) or die(mysql_error());

				console_output($status_value . ' ' . $total);
			}

			console_output("==============================");
		}
	}
}

if($task == 'get_domains')
{
	$records                = $argv[2];
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) {
		console_output("Getting email addresses.");
		$query 		= "SELECT `id`,`email`,`domain` FROM `emails` WHERE `domain` = '' ORDER BY `id` LIMIT ".$records;
		$result		= mysql_query($query) or die(mysql_error());
		$count		= mysql_num_rows($result);
		while($row = mysql_fetch_array($result)){
			$id					= $row['id'];
			$bits				= explode("@", $row['email']);
			$domains[]			= $bits[1];
			
			console_output("Processing ".$row['email'] . " | " . $row['domain']);
			
			mysql_query("UPDATE `emails` SET `domain` = '".$bits[1]."' WHERE `id` = '".$id."' ") or die(mysql_error());
		}
		
		echo "\n";
		
		console_output("Done getting email addresses.");
		
		console_output("Total Domains: " . number_format ( count ( $domains ) ) ) ;
		$domains = array_unique($domains);
		console_output("Unique Domains: " . number_format ( count ( $domains ) ) ) ;
		
		$count = count( $domains );
		
		foreach ( $domains as $domain ) {
			$input = mysql_query("INSERT IGNORE INTO `domains` 
				(`domain`, `status`)
				VALUE
				('".$domain."', 'active')") or die(mysql_error());

			console_output($count . ") ".$domain);

			$count = $count - 1;
		}
	}
}

if($task == 'get_domains_2')
{
	$records                = $argv[2];
	require $base.'../inc/cron.helper.php';
	if ( ( $pid = cronHelper::lock() ) !== FALSE ) {
		$query 		= "SELECT `id` FROM `emails` WHERE `domain` = '' ";
		$result		= mysql_query($query) or die(mysql_error());
		$remaining_records_to_process		= mysql_num_rows($result);
		
		console_output("Remaining records to process: ".number_format ( $remaining_records_to_process ) );
		
		console_output("Getting ".number_format ( $records ) ." email addresses.");
		
		$query 		= "SELECT `id`,`email`,`domain` FROM `emails` WHERE `domain` = '' ORDER BY `id` LIMIT ".$records;
		$result		= mysql_query($query) or die(mysql_error());
		$count		= mysql_num_rows($result);
		while($row = mysql_fetch_array($result)){
			$id					= $row['id'];
			$bits				= explode("@", $row['email']);
			$domains[]		= $bits[1];			
		}
		
		echo "\n";
		
		console_output("Done getting email addresses.");
		
		console_output("Total Domains: " . number_format ( count ( $domains ) ) ) ;
		$domains = array_unique($domains);
		console_output("Unique Domains: " . number_format ( count ( $domains ) ) ) ;
		
		$count = count( $domains );
		
		foreach ( $domains as $domain ) {
			mysql_query("UPDATE `emails` SET `domain` = '".$domain."' WHERE `email` LIKE '%@".$domain."' AND `domain` = '';
") or die(mysql_error());
			
			$input = mysql_query("INSERT IGNORE INTO `domains` 
				(`domain`, `status`, `last_checked`)
				VALUE
				('".$domain."', 'pending', '1')") or die(mysql_error());

			console_output ( number_format ( $count ) . ") ".$domain);

			$count = $count - 1;
		}
	}
}

console_output("Finished.");